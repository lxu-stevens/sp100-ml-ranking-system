...
def normalize_month_options(months) -> list[pd.Timestamp]:
    normalized = []
    for month in months:
        if pd.isna(month):
            continue
        normalized.append(pd.Timestamp(month))
...
def ensure_streamlit_runtime() -> None:
    if get_script_run_ctx() is None:
        raise SystemExit(
            "This app must be started with `streamlit run streamlit_app.py`, not `python streamlit_app.py`."
        )
...
def main():
    ...
    months = normalize_month_options(available_months_for_model(selected_df, model_name))
    if not months:
        st.error(f"No target months are available for model `{model_name}`.")
        st.stop()
    ...
    with right_col:
        ...
        st.code(metrics_row.get("classification_report") or "N/A")
