...
def l1_logistic_regression_factory(c: float = 0.5):
    return Pipeline([
        ("scaler", StandardScaler()),
        (
            "model",
...
def run_standard_models(ml_df: pd.DataFrame):
    ...
    l1_logistic_df, _, l1_logistic_metrics, X_test, _, y_pred_proba = train_test_model(
        df=ml_df,
        feature_columns=BASE_FEATURE_COLUMNS,
        model_factory=l1_logistic_regression_factory,
    )
    l1_logistic_predictions = build_test_prediction_frame(l1_logistic_df, X_test, y_pred_proba, "L1 Logistic Regression")
    l1_logistic_top20 = evaluate_top20_monthly_return(l1_logistic_df, X_test, y_pred_proba)
    l1_logistic_overall = l1_logistic_df.loc[X_test.index, "future_return_1m"].mean()
    l1_logistic_metrics.update(compute_portfolio_metrics(l1_logistic_predictions))
    print_standard_model_summary(
        model_name="L1 Logistic Regression",
        metrics=l1_logistic_metrics,
        top20_avg_return=l1_logistic_top20,
        overall_avg_return=l1_logistic_overall,
    )
    results.append({
        "model_name": "L1 Logistic Regression",
        "evaluation_name": "standard",
        ...
        "predictions": l1_logistic_predictions,
        "portfolio_series": build_portfolio_time_series(
            l1_logistic_predictions,
            model_name="L1 Logistic Regression",
            evaluation_name="standard",
        ),
    })
